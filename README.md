...
代码：[DeepResearch](https://github.com)
  ...
  def _run(self, data: str, model: str, **kwargs) -> List[List[Message]]:
      ...
      while num_llm_calls_available > 0:
          ...
          if token_count > max_tokens:
              ...
              messages[-1]['content'] = "You have now reached the maximum context length you can handle. You should stop making tool calls and, based on all the information above, think again and provide what you consider the most likely answer in the following format:your final thinking\nyour answer"
              content = self.call_server(messages, planning_port)
              messages.append({"role": "assistant", "content": content.strip()})
              # token数达到阈值后，成功返回结果
              if '' in content and '' in content:
...
* `tool_python`
  ...
  执行python代码。(code;Interpreter)→(stdout,stderr)(code;Interpreter)→(stdout,stderr)
...
搜索具体的url，并根据goal总结返回。(url,goal;π)→summary(url,goal;π)→summary
...
![image-20251017120415289](/Users/lehaoqu/Library/Application%20Support/typora-user-images/image-20251017120415289.png)
...
* `tool_search`
  ...
  调用google 进行search。(q;Enginer)→docs(q;Enginer)→docs
* `tool_scholar`
